...
- [x] Startup boot-check wired on **all 5 runnable V2.4 agents**: `crew-orchestrator`, `nemoclaw-agent`, `healer-agent` (agents.yml), `agent-factory` (registry.yml), `broski-bot` (core.yml, discord `on_ready`). Each calls `boot_check("<name>")` on startup, strict via `LOADOUT_STRICT`, fail-open on missing mount. All boot clean (0 missing); broski-bot correctly excludes forbidden HS-032.
- [x] **All 4 brain agents** — `agent-focus-tracker`, `agent-mcp-bridge`, `agent-morning-briefing` (in `__main__`), and `agent-hyper-brain-core` (in its module-level `@app.on_event("startup")`, since it runs via `uvicorn hyper_brain_core:app`) all boot-check on startup, using HYPER-SILLs' **own** `scripts/agent_boot.py` mounted at `/hyper-sills` (no copy — cross-repo module decision resolved cleanest). Fully fail-open. Mounts in V2.4 `docker-compose.brain.yml`. All resolve clean (7 required, 0 missing).
- [x] **ROLLOUT COMPLETE — boot-check live on all 9 runnable agents** (5 V2.4 + 4 brain), across 3 repos, on exactly 2 resolver implementations (V2.4 `agents/shared/loadout.py` + HYPER-SILLs `scripts/agent_boot.py`). No drift.
- [ ] `agent-registry`/`agent-spawner` have no `main.py` (not runnable Python — loadout entries are aspirational; nothing to wire).
