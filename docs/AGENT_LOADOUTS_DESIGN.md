...
- [x] **Brain agents (3 of 4)** — `agent-focus-tracker`, `agent-mcp-bridge`, `agent-morning-briefing` (separate BROski-Obsidian-Brain repo) call `boot_check(...)` in their `__main__` startup, using HYPER-SILLs' **own** `scripts/agent_boot.py` mounted at `/hyper-sills` (no copy — the cross-repo module decision, resolved cleanest). Fully fail-open. Mount + env in V2.4 `docker-compose.brain.yml`. All resolve clean (7 required, 0 missing).
- [ ] `agent-hyper-brain-core` — builds from the whole Brain repo (different structure than the `.agents/<name>` single-file agents); wire separately.  ← next
- [ ] `agent-registry`/`agent-spawner` have no `main.py` (not runnable Python — loadout entries are aspirational).
