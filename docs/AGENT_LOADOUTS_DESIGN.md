...
- [ ] **Brain agents** (separate BROski-Obsidian-Brain repo)  ← next. Needs a cross-repo module-sharing decision: the shared `loadout.py` lives in V2.4, but HYPER-SILLs already ships its own `scripts/agent_boot.py` (the canonical resolver) — mounting that into the brain agents is likely cleanest.
